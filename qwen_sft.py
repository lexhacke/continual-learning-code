...
class SDFTModule(L.LightningModule):
    ...
    Uses 8-bit quantization + LoRA for memory-efficient training.
    Teacher uses EMA of student's LoRA weights.
    """
    ...
    def __init__(
        ...
        # Student model with LoRA adapters (trainable)
        ...
        # Teacher LoRA weights (EMA of student's LoRA weights)
        # We store copies of just the LoRA parameters
        self.teacher_lora_state = {}
        self._init_teacher_lora()

        self.ema_decay = ema_decay
        self.max_new_tokens = max_new_tokens
        self.temperature = temperature

    def _init_teacher_lora(self):
        """Initialize teacher LoRA weights as copy of student's."""
        for name, param in self.student.named_parameters():
            if param.requires_grad:  # Only LoRA params
                self.teacher_lora_state[name] = param.data.clone()
...
Now generate the code that satisfies the specification above:"""
    ...
    def _update_ema(self):
        """Update teacher LoRA weights with EMA of student LoRA weights."""
        for name, param in self.student.named_parameters():
            if param.requires_grad and name in self.teacher_lora_state:
                self.teacher_lora_state[name].mul_(self.ema_decay).add_(
                    param.data, alpha=1 - self.ema_decay
                )

    @torch.no_grad()
    def _swap_to_teacher(self):
        """Temporarily swap student LoRA weights with teacher weights."""
        self._student_backup = {}
        for name, param in self.student.named_parameters():
            if param.requires_grad and name in self.teacher_lora_state:
                self._student_backup[name] = param.data.clone()
                param.data.copy_(self.teacher_lora_state[name])

    @torch.no_grad()
    def _swap_to_student(self):
        """Restore student LoRA weights after teacher forward pass."""
        for name, param in self.student.named_parameters():
            if param.requires_grad and name in self._student_backup:
                param.data.copy_(self._student_backup[name])
        self._student_backup = {}
    ...
    def training_step(self, batch: Dict[str, str], batch_idx: int) -> torch.Tensor:
        ...
        # Compute teacher log probs (swap to teacher LoRA weights)
        with torch.no_grad():
